...
/* 
 * Fix for macro conflicts between genbki.h (included by knl_variable.h)
 * and other headers or PG_MODULE_MAGIC.
 * genbki.h defines DATA/DESCR/SHDESCR which can conflict.
 */
#undef DATA
#undef DESCR
#undef SHDESCR
...
/* PG_MODULE_MAGIC must be outside extern "C" to avoid linkage issues */
PG_MODULE_MAGIC;

/* Entry points must have C linkage */
extern "C" {
    void _PG_init(void);
...
const static uint32 format_version_2 = 2;
const static uint32 format_version_1 = 1;
const static int nkeys_2 = 2;
const static size_t  len_3 = 3;
const static size_t  len_8 = 8;
const static size_t  len_9 = 9;
const static size_t count_3 = 3;

typedef struct {
    bool    insert;
    bool    update;
    bool    delete_gs;/* dup name with c++ keyword */
    bool    truncate;
} JsonAction;
...
static void columns_to_stringinfo(LogicalDecodingContext *ctx,
    TupleDesc tupdesc, HeapTuple tuple, bool addcomma, Oid reloid);
static void tuple_to_stringinfo(LogicalDecodingContext *ctx, TupleDesc tupdesc,
    HeapTuple tuple, TupleDesc indexdesc, bool replident, bool addcomma, Oid reloid);
static void pk_to_stringinfo(LogicalDecodingContext *ctx, TupleDesc tupdesc,
    HeapTuple tuple, TupleDesc indexdesc, bool addcomma);
...
static void pg_decode_write_tuple(LogicalDecodingContext *ctx, Relation relation, HeapTuple tuple,
    PGOutputJsonKind kind);
static void pg_decode_write_change(LogicalDecodingContext *ctx, ReorderBufferTXN *txn,
...
static Datum pg_lsn_out(PG_FUNCTION_ARGS);
/*
...
static Datum pg_lsn_out(PG_FUNCTION_ARGS)
{
    ...
    uint32  id;
    uint32  off;
    const static uint32 offset_helper = 32;
    /* Decode ID and offset */
    id = (uint32) (lsn >> offset_helper);
    off = (uint32) lsn;
...
static void pg_decode_startup(LogicalDecodingContext *ctx, OutputPluginOptions *opt, bool is_init)
{
    ...
    data = (JsonDecodingData*) palloc0(sizeof(JsonDecodingData));
    ...
    t = (SelectTable*)palloc0(sizeof(SelectTable));
    t->allschemas = true;
    t->alltables = true;
...
static void pg_decode_begin_txn(LogicalDecodingContext *ctx, ReorderBufferTXN *txn)
{
    ...
    if (data->format_version == format_version_2)
        pg_decode_begin_txn_v2(ctx, txn);
    else if (data->format_version == format_version_1)
        pg_decode_begin_txn_v1(ctx, txn);
    else
        elog(ERROR, "format version %d is not supported", data->format_version);
}

static void pg_decode_begin_txn_v1(LogicalDecodingContext *ctx, ReorderBufferTXN *txn)
{
    JsonDecodingData *data = (JsonDecodingData*)ctx->output_plugin_private;
    ...
    if (data->include_timestamp)
        appendStringInfo(ctx->out, "%s\"timestamp\":%s\"%s\",%s", data->ht, data->sp,
            timestamptz_to_str(txn->commit_time), data->nl);
...
static void pg_decode_begin_txn_v2(LogicalDecodingContext *ctx, ReorderBufferTXN *txn)
{
    JsonDecodingData *data = (JsonDecodingData*)ctx->output_plugin_private;
...
/* COMMIT callback */
static void pg_decode_commit_txn(LogicalDecodingContext *ctx, ReorderBufferTXN *txn,
    XLogRecPtr commit_lsn)
{
    ...
    elog(DEBUG2, "my change counter: " UINT64_FORMAT " ; # of changes: " UINT64_FORMAT \
    " ; # of changes in memory: " UINT64_FORMAT, data->nr_changes, txn->nentries, txn->nentries_mem);
...
static void pg_decode_commit_txn_v1(LogicalDecodingContext *ctx, ReorderBufferTXN *txn,
    XLogRecPtr commit_lsn)
{
    JsonDecodingData *data = (JsonDecodingData*)ctx->output_plugin_private;
...
static void pg_decode_commit_txn_v2(LogicalDecodingContext *ctx, ReorderBufferTXN *txn,
    XLogRecPtr commit_lsn)
{
    JsonDecodingData *data = (JsonDecodingData*)ctx->output_plugin_private;
    ...
    HeapTuple tuple, TupleDesc indexdesc, bool replident, bool addcomma, Oid reloid)
{
    ...
    if (replident) {
        ...
        if (data->include_type_oids) {
            appendStringInfo(&coltypeoids, "%s%s%s%s\"keytypeoids\":%s[", data->ht, data->ht,
                data->ht, data->ht, data->sp);
        }
...
#endif 
{
        ...
        if (isnull) {
            ...
            switch (typid) {
                ...
                case FLOAT8OID:
                case NUMERICOID:
                    if (pg_strncasecmp(outputstr, "NaN", len_3) == 0 ||
                            pg_strncasecmp(outputstr, "Infinity", len_8) == 0 ||
                            pg_strncasecmp(outputstr, "-Infinity", len_9) == 0) {
                        appendStringInfo(&colvalues, "%snull", comma);
                        ...
                        if ((*outputstr)=='.') {
                            appendStringInfo(&colvalues, "%s0%s", comma, outputstr);
                        } else {
        ...
        if (strcmp(comma, "") == 0)
            snprintf(comma, count_3, ",%s", data->sp);
    }
...
static void columns_to_stringinfo(LogicalDecodingContext *ctx, TupleDesc tupdesc, HeapTuple tuple,
    bool addcomma, Oid reloid)
{
    tuple_to_stringinfo(ctx, tupdesc, tuple, NULL, false, addcomma, reloid);
}

/* Print replica identity information */
static void identity_to_stringinfo(LogicalDecodingContext *ctx, TupleDesc tupdesc,
    HeapTuple tuple, TupleDesc indexdesc)
{
...
static void pk_to_stringinfo(LogicalDecodingContext *ctx, TupleDesc tupdesc, HeapTuple tuple,
    TupleDesc indexdesc, bool addcomma)
{
    JsonDecodingData    *data;
    int                    natt;
    char                comma[3] = "";
    ...
    for (natt = 0; natt < tupdesc->natts; natt++) {
         ...
         * this macro, use it. Version 10 and later already support it.
         */
#if (PG_VERSION_NUM >= 90600 && PG_VERSION_NUM < 90605) || (PG_VERSION_NUM >= 90500 && PG_VERSION_NUM < 90509) || \
...
static void pg_decode_change_v1(LogicalDecodingContext *ctx, ReorderBufferTXN *txn,
    Relation relation, ReorderBufferChange *change)
{
    JsonDecodingData *data;
    ...
    if (change->action == REORDER_BUFFER_CHANGE_DELETE && !data->actions.delete_gs) {
    ...
    if (list_length(data->filter_tables) > 0) {
        ...
        foreach(lc, data->filter_tables) {
            SelectTable    *t = (SelectTable*)lfirst(lc);
    ...
    switch (change->action) {
        case REORDER_BUFFER_CHANGE_INSERT:
            appendStringInfo(ctx->out, "%s%s%s\"kind\":%s\"insert\",%s", data->ht, data->ht,
                data->ht, data->sp, data->nl);
            break;
        case REORDER_BUFFER_CHANGE_UPDATE:
            appendStringInfo(ctx->out, "%s%s%s\"kind\":%s\"update\",%s", data->ht, data->ht,
                data->ht, data->sp, data->nl);
            break;
        case REORDER_BUFFER_CHANGE_DELETE:
            appendStringInfo(ctx->out, "%s%s%s\"kind\":%s\"delete\",%s", data->ht, data->ht,
                data->ht, data->sp, data->nl);
            break;
        default:
            Assert(false);
    ...
    switch (change->action) {
        ...
        case REORDER_BUFFER_CHANGE_UPDATE:
            /* Print the new tuple */
                        appendStringInfo(ctx->out, ",%s", data->nl);
            columns_to_stringinfo(ctx, tupdesc, &change->data.tp.newtuple->tuple,
                true, change->data.tp.relnode.relNode);
            if (data->include_pk && pkrel != NULL)
            ...
            break;
        case REORDER_BUFFER_CHANGE_DELETE:
                        if (change->data.tp.oldtuple == NULL) {break;};
                        appendStringInfo(ctx->out, ",%s", data->nl);
            if (data->include_pk && pkrel != NULL)
                pk_to_stringinfo(ctx, tupdesc, &change->data.tp.newtuple->tuple, pkdesc, true);
...
static void pg_decode_write_value(LogicalDecodingContext *ctx, Datum value, bool isnull, Oid typid)
{
    ...
    switch (typid) {
        ...
        case FLOAT8OID:
        case NUMERICOID:
            if (pg_strncasecmp(outstr, "NaN", len_3) == 0 ||
                    pg_strncasecmp(outstr, "Infinity", len_8) == 0 ||
                    pg_strncasecmp(outstr, "-Infinity", len_9) == 0) {
                appendStringInfoString(ctx->out, "null");
...
static void pg_decode_write_tuple(LogicalDecodingContext *ctx,
    Relation relation, HeapTuple tuple, PGOutputJsonKind kind)
{
    JsonDecodingData    *data;
    TupleDesc            tupdesc;
    Relation            defrel = NULL;
    ...
    if (kind == PGOUTPUTJSON_IDENTITY) {
        ...
        if (OidIsValid(relation->rd_replidindex))    /* 10+ */ {
            idxrel = RelationIdGetRelation(relation->rd_replidindex);
            idxdesc = RelationGetDescr(idxrel);
        }
    }
...
(PG_VERSION_NUM >= 90400 && PG_VERSION_NUM < 90414)
        ...
        if (kind == PGOUTPUTJSON_PK ||
            (kind == PGOUTPUTJSON_IDENTITY && relation->rd_rel->relreplident != REPLICA_IDENTITY_FULL)) {
    ...
    Relation relation, ReorderBufferChange *change)
{
    ...
    switch (change->action) {
        case REORDER_BUFFER_CHANGE_INSERT:
            if (change->data.tp.newtuple == NULL) {
                elog(WARNING, "no tuple data for INSERT in table \"%s\".\"%s\"",
        ...
        case REORDER_BUFFER_CHANGE_UPDATE:
            if (change->data.tp.newtuple == NULL) {
                elog(WARNING, "no tuple data for UPDATE in table \"%s\".\"%s\"",
                    get_namespace_name(RelationGetNamespace(relation)),
                    RelationGetRelationName(relation));
                return;
            }
            if (change->data.tp.oldtuple == NULL) {
                if (!OidIsValid(relation->rd_replidindex) && relation->rd_rel->relreplident != REPLICA_IDENTITY_FULL) {
                    elog(WARNING, "no tuple identifier for UPDATE in table \"%s\".\"%s\"",
                        get_namespace_name(RelationGetNamespace(relation)),
                        RelationGetRelationName(relation));
                    return;
                }
        ...
        case REORDER_BUFFER_CHANGE_DELETE:
            if (change->data.tp.oldtuple == NULL) {
                if (!OidIsValid(relation->rd_replidindex) && relation->rd_rel->relreplident != REPLICA_IDENTITY_FULL) {
                    elog(WARNING, "no tuple identifier for DELETE in table \"%s\".\"%s\"",
                        get_namespace_name(RelationGetNamespace(relation)),
                        RelationGetRelationName(relation));
                        return;
                }
    ...
    if (change->data.tp.oldtuple != NULL) {
        ...
        if (change->action == REORDER_BUFFER_CHANGE_UPDATE) {
...
#endif 
            {
                ...
                elog(WARNING, "no old tuple data for UPDATE in table \"%s\".\"%s\"",
                    get_namespace_name(RelationGetNamespace(relation)),
                    RelationGetRelationName(relation));
            }
        }
        ...
        if (change->action == REORDER_BUFFER_CHANGE_DELETE) {
            elog(WARNING, "no old tuple data for DELETE in table \"%s\".\"%s\"",
                get_namespace_name(RelationGetNamespace(relation)),
                RelationGetRelationName(relation));
        }
    }
...
static void pg_decode_change_v2(LogicalDecodingContext *ctx, ReorderBufferTXN *txn,
    Relation relation, ReorderBufferChange *change)
{
    JsonDecodingData *data = (JsonDecodingData*) ctx->output_plugin_private;
    MemoryContext old;
...
static void pg_decode_message(LogicalDecodingContext *ctx, ReorderBufferTXN *txn,
...
static void pg_decode_message_v1(LogicalDecodingContext *ctx, ReorderBufferTXN *txn,
    XLogRecPtr lsn, bool transactional, const char *prefix, Size
    content_size, const char *content)
{
    JsonDecodingData *data;
    ...
    appendStringInfo(ctx->out, "{%s%s%s%s\"kind\":%s\"message\",%s", data->nl, data->ht, data->ht,
        data->ht, data->sp, data->nl);

    if (transactional)
        appendStringInfo(ctx->out, "%s%s%s\"transactional\":%strue,%s", data->ht, data->ht,
            data->ht, data->sp, data->nl);
    else
        appendStringInfo(ctx->out, "%s%s%s\"transactional\":%sfalse,%s", data->ht, data->ht,
            data->ht, data->sp, data->nl);
...
static void pg_decode_message_v2(LogicalDecodingContext *ctx, ReorderBufferTXN *txn,
    XLogRecPtr lsn, bool transactional, const char *prefix, Size
    content_size, const char *content)
{
    JsonDecodingData    *data = (JsonDecodingData*)ctx->output_plugin_private;
    ...
    ReorderBufferChange *change)
{
    ...
    appendStringInfo(ctx->out, "{%s%s%s%s\"kind\":%s\"truncate\",%s", data->nl, data->ht, data->ht,
        data->ht, data->sp, data->nl);

    if (data->include_xids)
        appendStringInfo(ctx->out, "%s%s%s\"xid\":%s%lu,%s", data->ht, data->ht, data->ht,
            data->sp, txn->xid, data->nl);

    if (data->include_timestamp)
        appendStringInfo(ctx->out, "%s%s%s\"timestamp\":%s\"%s\",%s", data->ht, data->ht,
            data->ht, data->sp, timestamptz_to_str(txn->commit_time), data->nl);

    if (data->include_origin)
        appendStringInfo(ctx->out, "%s%s%s\"origin\":%s%u,%s", data->ht, data->ht, data->ht,
            data->sp, txn->origin_id, data->nl);

    if (data->include_lsn) {
        char *lsn_str = DatumGetCString(DirectFunctionCall1(pg_lsn_out, UInt64GetDatum(change->lsn)));
        appendStringInfo(ctx->out, "%s%s%s\"lsn\":%s\"%s\",%s", data->ht, data->ht,
            data->ht, data->sp, lsn_str, data->nl);
        pfree(lsn_str);
    }
...
static bool parse_table_identifier(List *qualified_tables, char separator, List **select_tables)
{
    ...
    foreach(lc, qualified_tables) {
        ...
        char        *nextp;
        int            len;
        SelectTable    *t = (SelectTable*)palloc0(sizeof(SelectTable));
