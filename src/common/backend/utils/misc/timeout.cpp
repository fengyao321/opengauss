 ...
 * as needed.
 */
void pg_handle_sig_alarm(SIGNAL_ARGS)
{
...
void InitializeTimeouts(void)
{
    ...
    pqsignal(SIGALRM, pg_handle_sig_alarm);
}
...
TimeoutId RegisterTimeout(TimeoutId id, timeout_handler_proc handler)
{
    ...
    if ((TimeoutId)iid >= USER_TIMEOUT) {
        /* Allocate a user-defined timeout reason */
        for (iid = (int)USER_TIMEOUT; (TimeoutId)iid < MAX_TIMEOUTS; iid++)
            if (all_timeouts[iid].timeout_handler == NULL)
                break;
        if ((TimeoutId)iid >= MAX_TIMEOUTS)
            ereport(FATAL,
                    (errcode(ERRCODE_CONFIGURATION_LIMIT_EXCEEDED),
                     errmsg("cannot add more timeout reasons")));
    }

    Assert(all_timeouts[iid].timeout_handler == NULL);

    all_timeouts[iid].timeout_handler = handler;

    return (TimeoutId)iid;
}
 ...
 * of a SIGALRM event (due to longjmp'ing out of pg_handle_sig_alarm before it
