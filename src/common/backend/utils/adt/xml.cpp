...
static void XmlTableInitOpaque(TableFuncScanState *state, int natts)
{
    ...
    xtCxt = (XmlTableBuilderData*)palloc0(sizeof(XmlTableBuilderData));
    xtCxt->magic = XMLTABLE_CONTEXT_MAGIC;
    xtCxt->natts = natts;
    xtCxt->xpathscomp = (xmlXPathCompExprPtr*)palloc0(sizeof(xmlXPathCompExprPtr) * natts);
...
static void XmlTableSetDocument(TableFuncScanState *state, Datum value)
{
     ...
     * comment in xml_out.
     */
    str = xml_out_internal(xmlval, (pg_enc)0);

    length = strlen(str);
    xstr = xml_charpnstrdup(str, length);
 ...
 */
static void XmlTableSetNamespace(TableFuncScanState *state, char *name, char *uri)
{
#ifdef USE_LIBXML
    XmlTableBuilderData *xtCxt;
    ...
    if (xmlXPathRegisterNs(xtCxt->xpathcxt,
                           xml_charpnstrdup(name, strlen(name)),
                           xml_charpnstrdup(uri, strlen(uri))))
...
static void XmlTableSetRowFilter(TableFuncScanState *state, char *path)
{
    ...
    xstr = xml_charpnstrdup(path, strlen(path));
 ...
 */
static void XmlTableSetColumnFilter(TableFuncScanState *state, char *path, int colnum)
{
#ifdef USE_LIBXML
    XmlTableBuilderData *xtCxt;
    xmlChar    *xstr;
...
static bool XmlTableFetchRow(TableFuncScanState *state)
{
     ...
     */
    xmlSetStructuredErrorFunc((void *) xtCxt->xmlerrcxt, xml_error_handler);
