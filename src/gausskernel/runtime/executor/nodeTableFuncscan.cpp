 ...
 * ----------------------------------------------------------------
 */
TupleTableSlot *ExecTableFuncScan(PlanState *state)
{
    TableFuncScanState *node = castNode(TableFuncScanState, state);
...
TableFuncScanState *ExecInitTableFuncScan(TableFuncScan *node, EState *estate, int eflags)
{
    ...
    scanstate->in_functions = (FmgrInfo*)palloc(sizeof(FmgrInfo) * tupdesc->natts);
    scanstate->typioparams = (Oid*)palloc(sizeof(Oid) * tupdesc->natts);
...
static void tfuncFetchRows(TableFuncScanState *tstate, ExprContext *econtext)
{
    ...
    tstate->tupstore = tuplestore_begin_heap(false, false, u_sess->attr.attr_memory.work_mem);
...
static void tfuncInitialize(TableFuncScanState *tstate, ExprContext *econtext, Datum doc)
{
    ...
    foreach(lc1, tstate->colexprs) {
        ...
        if (colno != ordinalitycol) {
            ExprState  *colexpr = (ExprState*)lfirst(lc1);
