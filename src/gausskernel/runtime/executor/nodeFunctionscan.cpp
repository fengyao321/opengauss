...
static TupleTableSlot* FunctionNext(FunctionScanState* node)
{
    ...
    if (tuplestorestate == NULL) {
        node->tuplestorestate = tuplestorestate = ExecMakeTableFunctionResult(
            node->funcexpr, node->ss.ps.ps_ExprContext, node->func_tupdesc, node->eflags & EXEC_FLAG_BACKWARD, node);
    }

    /*
     * Get the next tuple from tuplestore. Return NULL if no more tuples.
     */
    (void)tuplestore_gettupleslot(tuplestorestate, ScanDirectionIsForward(direction), false, funcslot);
    ...
    if (!TupIsNull(funcslot)) {
        ...
        heap_slot_getallattrs(funcslot);
...
FunctionScanState* ExecInitFunctionScan(FunctionScan* node, EState* estate, int eflags)
{
     ...
     */
    functypclass = get_expr_result_type(node->funcexpr, &funcrettype, &func_tupdesc);
    if (functypclass == TYPEFUNC_COMPOSITE) {
        ...
        if (func_tupdesc->td_tam_ops != TableAmHeap) {
             ...
             */
            int i;
            for (i = 0; i < func_tupdesc->natts; i++) {
                func_tupdesc->attrs[i].attcacheoff = -1;
            }
            func_tupdesc->td_tam_ops = TableAmHeap;
        }
        ...
        TupleDescInitEntry(func_tupdesc, (AttrNumber)1, attname, funcrettype, -1, 0);
        TupleDescInitEntryCollation(func_tupdesc, (AttrNumber)1, exprCollation(node->funcexpr));
