...
-- A-format databases always enforce primary-key and non-primary-key unique constraints.
...
-- B-format databases only allow unique_checks to control non-primary-key unique constraints.
...
-- With unique_checks enabled, both primary-key and non-primary-key unique constraints are enforced.
...
-- Primary keys are still enforced, while non-primary-key unique constraints can be bypassed.
...
-- A non-owner cannot bypass a non-primary-key unique constraint.
